...
class PdoGsb {
     ...
     */
    public function getInfosVisiteur($login, $mdp) {//cette fonction verifie si le login et le mdp se trouvent ds la bdd et kel est le nom et le prenom de cet utilisateur.
     ...
     * @return \l'id,
     */
    public function getInfosComptable($login, $mdp) {//cette fonction verifie si le login et le mdp se trouvent ds la bdd et kel est le nom et le prenom de cet utilisateur.
        $requetePrepare = PdoGsb::$monPdo->prepare(
    ...
    /* public function getInfosSecretaire($login, $mdp)//cette fonction verifie si le login et le mdp se trouvent ds la bdd et kel est le nom et le prenom de cet utilisateur.
      {
      $requetePrepare = PdoGsb::$monPdo->prepare(
      'SELECT secretaire.id AS id, secretaire.nom AS nom, '
      . 'secretaire.prenom AS prenom '
      . 'FROM secretaire '
      . 'WHERE secretaire.login = :unLogin AND secretaire.mdp = :unMdp'
      );
      $requetePrepare->bindParam(':unLogin', $login, PDO::PARAM_STR);
      $requetePrepare->bindParam(':unMdp', $mdp, PDO::PARAM_STR);
      $requetePrepare->execute();
    ...
    /*public function majFraisHorsForfait($idVisiteur, $idFrais, $montant, $libelle, $mois) {
        ...
        . 'SET lignefraishorsforfait.montant = :unMontant ,lignefraishorsforfait.libelle = :unLibelle '
        . 'WHERE lignefraishorsforfait.idvisiteur = :unIdVisiteur '
